...
def jlog(event: str, **fields):
    # Structured JSON logs for Docker-friendly stdout
    payload = {"event": event, **fields}
    try:
        print(json.dumps(payload, ensure_ascii=False), flush=True)
    except Exception:
        # Never break runtime on logging failures
...
def log_config_startup():
    # Avoid printing secrets; redact DATABASE_URL password if present
    db = os.getenv("DATABASE_URL", "")
    redacted = db
    if "://" in db and "@" in db:
        # crude but safe: redact between '://' and '@' after first ':'
        try:
...
def health_deps():
    ...
    Never raises: always returns JSON describing dependency status.
    """
    deps = {}
...
def ask(req: AskRequest):
    try:
        ...
        jlog("ask_done", trace_id=trace_id, user_id=user_id, latency_ms=latency_ms, used_context=bool(result.get('used_context')), used_conversation_context=bool(result.get('used_conversation_context')), num_docs=int(result.get('num_docs') or 0), target_expert=(result.get('target_expert') or 'general'), tool=result.get('tool'))
